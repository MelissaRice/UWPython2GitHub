...
purpose: Modify the 30 minute webserver as follows:
...
def get_content(uri):
    ...
    try:
        ...
        elif path[-3:len(path)] == ".py":
            if diagnostics:
                print "Attempting to run this python file: %s" % path
            try:
                pyString = "python %s" % path
                process = sub.Popen(pyString,stdout=sub.PIPE,stderr=sub.PIPE)
                (output, errors) = process.communicate()
                if errors:
                    errorMessage = "Error running %s: %s" % (path, errors)
                    return 498, 'text/html', errorMessage
                else:                    
                    return 200, 'text/html', output
            except Exception, err:
                errorMessage = "Error running your code %s: %s" % (path, err)
                return 498, 'text/html', errorMessage
